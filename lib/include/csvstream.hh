...
public:
  ...
  // Constructor from filename.  Throws a csvstream_exception if filename cannot
  // be opened or if the CSV header cannot be read.
  csvstream(const std::string &filename, char delimiter=',');

  // Constructor from stream.  Throws a csvstream_exception if the CSV header
  // cannot be read.
  ...
  // Stream extraction operator reads one row.  Throws a csvstream_exception if
  // the row is improperly formatted.
